#ifndef COMMON_H
#define COMMON_H

#include <U8g2lib.h>

// fonts
#define FONT_CHN_16       u8g2_font_wqy16_t_gb2312b   // (16=18x18) 16号中文字体 (12=12x13, 13=12x14, 14=13x16)
#define FONT_INPUT_DIGIT  u8g2_font_logisoso16_tn     // (9x19) 输入数值字体
#define FONT_DISP_SMALL   u8g2_font_fub14_tn          // (21x26) 显示数值字体 中号
#define FONT_DISP_MID     u8g2_font_fub20_tn          // (28x36) 显示数值字体 中号
#define FONT_DISP_LARGE   u8g2_font_fub25_tn          // (35x46) 显示数值字体 大号
#define FONT_NARROW_SMALL u8g2_font_busdisplay11x5_tr // (5x13) 窄字体 小号
#define FONT_SMALL_ENG    u8g2_font_profont11_tf      // (6x11) 小英文数字
#define FONT_SMALL_ENG2   u8g2_font_bitcasual_tr      // (11x11) 小英文数字

#endif // COMMON_H